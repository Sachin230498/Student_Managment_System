...
// import Addcourses from './Addcourses';
// import Addoncourse from './Addoncourse';
// import CourseForm from './CourseForm'; // Assuming you'll create a form component for editing courses
// import { Link } from 'react-router-dom';
...
//                   <td className="p-2 border flex justify-center space-x-2" >
//                     <Link to={`/courseContent/${course._id}`} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700"  >View</Link>
//                     <button onClick={() => setShowAddCourse(true)} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-700">Add content </button>
//                     <button onClick={() => handleDeleteCourse(course._id)} className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-700">Delete</button>
//                     <button onClick={() => handleEditCourse(course)} className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-700">Edit</button>
//                   </td>
//                 </tr>
//               ))}
//             </tbody>
...
//         {showEditCourse && <CourseForm onClose={() => setShowEditCourse(false)} course={editingCourse} onUpdateCourse={handleUpdateCourse} />}
//       </div>
//     </div>
//   );
// }
...
export default function Courses() {
  ...
  const handleAddContent = (courseId) => {
    ...
    console.log(courseId); // Log the courseId to verify it's correct
    // Perform additional logic related to adding content here
  };
  ...
  return (
    <div className="flex flex-col md:flex-row">
      ...
      <div className="flex flex-col flex-1 p-4 md:p-6">
            ...
            <tbody>
              {courses.map((course) => (
                <tr key={course._id}>
                  ...
                  <td className="p-2 border flex justify-center space-x-2" >
                    <Link to={`/courseContent/${course._id}`} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700"  >View</Link>
                    <button onClick={() => {setShowAddCourse(true); handleAddContent(course._id);}}    className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-700">Add content </button>
                    <button onClick={() => handleDeleteCourse(course._id)} className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-700">Delete</button>
                    <button onClick={() => handleEditCourse(course)} className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-700">Edit</button>
                  </td>
                </tr>
        ...
        {showAddCourses && <Addcourses onClose={() => setShowAddCourses(false)} onAddCourse={handleAddCourse} />}
        {showAddCourse && <Addoncourse onClose={() => setShowAddCourse(false)} onAddContent={handleAddContent } />}
        {showEditCourse && <CourseForm onClose={() => setShowEditCourse(false)} course={editingCourse} onUpdateCourse={handleUpdateCourse} />}
      </div>
    </div>
  );
